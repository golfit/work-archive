#This script reads in a shot list and setups of the tree nodes to hold transfer function calculations at all frequencies between the given signal nodes and the antenna current signal.
...
import re
#from calcTransFun import * #Don't need this
...
for s in sList :
    ...
    for nodeName in nodeList :
        ...
        commentStr='Transfer function is calculated between ' + probeName +' and ' +str(antNode.getFullPath())
        ...
        try :
            ...
            topNode.addNode('tfun_allfreq','structure')

            #Write changes to tree.
            magTree.write()
        except :
            print("...can't add tf structure node - may exist already")

        tfNode=topNode.getNode('tfun_allfreq')

        try :
            #Add a node for each coil
            tfNode.addNode(probeName,'numeric')

            #Add sub-nodes for node.
            n=tfNode.getNode(probeName)
            
            n.addNode('t_range','numeric')
            n.addNode('freq_range','numeric')
            n.addNode('n_time_pts','numeric')
            n.addNode('n_freq_pts','numeric')
            n.addNode('Hxy_r','numeric')
            n.addNode('Hxy_i','numeric')
        ...
        print("Setup magnitude-squared cross transfer function nodes in "+n.getPath()+" for Shot {0:d}".format(s))
