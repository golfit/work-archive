#This script reads in a shot list and processes the cross power spectrogram between the given nodes and the antenna current signal.
#Ted Golfinopoulos, 16 Aug 2012
#See also: ProcessCrossCohereNoAdd.py, myTools.py, SetupCrossPowerNodes.py SetupCrossCohereNodes.py
...
for s in sList :
	...
	for nodeName in nodeList :
		...
		#Calculate magnitude-squared cross-power spectrogram
		nfft=2048
		Cxy,f,t=myTools.crossSpecgram(ia[numpy.logical_and(ti>t1,ti<t2)],y,nfft,fs)
		...
		n.getNode('Pxy_r').putData(Data.compile("real("+n.getPath()+")"))
		n.getNode('Pxy_i').putData(Data.compile("aimag("+n.getPath()+")"))
	
		print("Put cross power data in "+n.getPath()+" for Shot {0:d}".format(s))
