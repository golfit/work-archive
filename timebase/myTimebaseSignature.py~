...
def timeSignatureAnalyze(y, start,discriminator=1) :
    """
    JAS  1/27/11
    ...
    TED GOLFINOPOULOS - 24 MAY 2013 - Just added discriminator as an optional argument
     """
    ...
    rising = (transitions > discriminator).nonzero()[0]
    falling = (transitions < -discriminator).nonzero()[0]
...
def timeSignature(parent,discriminator=1) :
    '''
      timeSignature
    ...
    if gates and gates.length==4 :
        ...
        (times, idxs) = timeSignatureAnalyze(y, start,discriminator)
