...
import numpy as np
from geqdsk_utils import file_numbers, writef
...
def write(f, data):
    ...
    item_count=writef(f, data['rdim'])
    item_count=writef(f, data['zdim'],item_count)
    item_count=writef(f, data['rcentr'],item_count)
    item_count=writef(f, data['rgrid1'],item_count)
    item_count=writef(f, data['zmid'],item_count)
    #f.write('\n')#Newline
    
    item_count=writef(f, data['rmagx'],item_count)
    item_count=writef(f, data['zmagx'],item_count)
    item_count=writef(f, data['simagx'],item_count)
    item_count=writef(f, data['sibdry'],item_count)
    item_count=writef(f, data['bcentr'],item_count)
    #f.write('\n')#Newline
    
    item_count=writef(f, data['cpasma'],item_count)
    item_count=writef(f, data['simagx'],item_count)
    item_count=writef(f, 0.0,item_count)
    item_count=writef(f, data['rmagx'],item_count)
    item_count=writef(f, 0.0,item_count)
    #f.write('\n')#Newline
    ...
    # Write the arrays
    item_count=writef(f, data['fpol'],item_count)
    item_count=writef(f, data['pressure'],item_count)
    item_count=writef(f, np.zeros(nx),item_count)  # Workk1
    item_count=writef(f, np.zeros(nx),item_count)  # Workk2
    item_count=writef(f, data['psi'],item_count)
    item_count=writef(f, data['qpsi'],item_count)
    
    #Write boundary information
    #f.write('\n---------------------\n')#Newline
    #Print size of boundary and limiters as strings, instead of floats.
    f.write(str(data['nbdry'])+ ' '+str(data['nlim'])+'\n')#'\n---------------------\n')#Newline
    #item_count=writef(f, data['nbdry'],item_count)
    #item_count=writef(f, data['nlim'],4) #Force newline
    #f.write('\n---------------------\n')#Newline
    item_count=writef(f, data['rbdry'],item_count)
    #f.write('\n---------------------\n')#Newline
    item_count=writef(f, data['zbdry'],item_count)
    #f.write('\n---------------------\n')#Newline
    #f.write('\n---------------------\n')#Newline
    item_count=writef(f, data['xlim'],item_count)
    #f.write('\n---------------------\n')#Newline
    item_count=writef(f, data['ylim'],item_count)
