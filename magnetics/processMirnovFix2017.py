...
for s in sList :
    ...
    for n_name in fixAbkNodes:
        ...
        try :
            ...
            abkNodeState.append(n.isOn())
            if not n.isOn() :
                n.setOn(True) #Turn all the nodes on for re-processing
            rawNode=n.getNode('raw')
            print(rawNode.getFullPath())
            #print(Data.compile(str(rawNode.getData()).replace('ACQ_216_2','ACQ_216_3')))#This is broken because MDSplus was "upgraded"
            ...
            if not abkNodeState[idx]:
                n.setOn(abkNodeState[idx])
        except : #Can't find node
            print("Can't fix "+n_name)
            continue
    ...
    for n_name in setPosSignNodes:
        try :
            ...
            print("Can't change sign for "+n_name)
    print('...done')
    ###
