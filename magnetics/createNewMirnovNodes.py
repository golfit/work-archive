...
def updateNodeInfo(s) :
    ...
    #Maps the index of the data value in the old setup (KA_TOP and KA_BOT) to the new setup (GH_BOT and GH_TOP)
    ghTopInd=1
    ghBotInd=0
    ...
    insertNewData(myTree.getNode('\magnetics::top.low_n_coils:nodename'),replaceInds,[topName,botName])
    ...
    for ii in range(len(oldNodes)):
        ...
        newNodes[replaceInds[ii]].putData( Data.compile(newData) )
