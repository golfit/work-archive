...
#def getShotRange(firstShot=None, lastShot=None) :
#    '''
#    Return list of valid shots from C-Mod given first and last shot in sys.argv.  If no last shot is given, just the single, given shot is returned.
#    USAGE:
#        shotList=getShotRange() #Grab arguments from sys.argv[1:2]
#        shotList=getShotRange(firstShot) #Only return a single shot, even if it is not in the Logbook
#        shotList=getShotRange(firstShot,lastShot) #Return range specified by given endpoints.
#    Ted Golfinopoulos, 27 May 2013 (created)
#    '''
#
#    if(firstShot is None) :
#        firstShot=int(sys.argv[1])
#
#    if(lastShot is None) :
#        if(len(sys.argv)>2) :
#            if(sys.argv[2]=='all') :
#            lastShot=1110329038 #Last shot for which PCI timebase signal was available, according to binary search survey 
#            else :
#            lastShot=int(sys.argv[2])
#        else :
#            lastShot=firstShot #Only examine and calculate timebase for single specified shot
#
#    #Fetch all shots in range.
#    if(firstShot!=lastShot) :
#        connection=pymssql.connect(host='alcdb2',database='logbook', user='golfit', password='pfcworld')
#        cur=connection.cursor()
#        cur.execute('select shot from shots where shot>={0:d} and shot <={1:d} order by shot'.format(firstShot,lastShot))
#        ans=cur.fetchall()
...
def parsePremadeNodeList(stringArgs, s) :
    ...
    if(any([re.match(".*reflect.*",x.lower())!=None for x in stringArgs])) :
        print('50, 60, 75, 88 GHz Low, 88 GHz High, 110 GHz, 132 GHz, and 140 GHz reflectometer channels will be processed.')
        nodePath='\\CMOD::TOP.ELECTRONS.REFLECT.CPCI:DT132_1:' #Reference C-Mod tree so can pull data when magnetics tree is opened.
        
        reflectNodes=[]
        for i in range(1,16+1,2) :
            #Make TDI expressions
            cosPath=nodePath+'input_{0:>02}'.format(i)
            sinPath=nodePath+"input_{0:>02}".format(i+1)
            thisPhase='_Acos='+cosPath+', _Asin='+sinPath+', _pol=cmplx(_Acos,_Asin),_phase=arg(_pol)'
            nodeExprAmp=thisPhase+', _amp=_Acos*cos(_phase)+_Asin*sin(_phase), Build_Signal(DATA($VALUE),_amp,\\cmod::top.electrons.reflect.cpci:dt132_1:t_sig_base)'
            nodeExprPhase=thisPhase+', Build_Signal(DATA($VALUE),_phase,\\cmod::top.electrons.reflect.cpci:dt132_1:t_sig_base)'
            nodeExprCos='_Acos='+cosPath+', Build_Signal(DATA($VALUE),_Acos, \\cmod::top.electrons.reflect.cpci:dt132_1:t_sig_base)'
            nodeExprSin='_Asin='+sinPath+', Build_Signal(DATA($VALUE),_Asin, \\cmod::top.electrons.reflect.cpci:dt132_1:t_sig_base)'

            reflectNodes.append(nodeExprAmp)
            reflectNodes.append(nodeExprPhase)
            reflectNodes.append(nodeExprCos)
            reflectNodes.append(nodeExprSin)
            #        reflectNodes=[for x in range(1,16+1,2)]
        nodeList=nodeList + reflectNodes #Concatenate pci nodes onto node list.

    if(any([re.match("gpi",x.lower())!=None for x in stringArgs])) :
        print('Processing GPI')
        #Number of rows=9, number of columns=10
        for i in range(0,9) :
            for j in range(0,10) :
                node_name='gpi_'+str(i)+'_'+str(j)
                node_expr='Build_Signal(DATA($VALUE)['+str(i)+','+str(j)+',*], \\CMOD::TOP.spectroscopy.gpi.apd_array.frames,\\cmod::top.SPECTROSCOPY.GPI.APD_ARRAY.HARDWARE:DT132_1:T_SIG_BASE )' #Reference C-Mod tree so can pull data when magnetics tree is opened.  Reference particular channel in GPI.
                #nodeList.append(node_expr) #Concatenate GPI node onto list of nodes.
                nodeList=nodeList+[node_expr]

    return nodeList,ceceNodes
    
def parseNodeNames(stringArgs) :
    '''
    Return single regular expression that will select for signal processing nodes under
    \magnetics::top.shoelace.cohere, tfun_allfreq, and trans_fun.  The regular expressions will
    only work if the node names are dropped to all lower case.
    
    USAGE:
        parseNodeNames(stringArgs)
    
    @arg stringArgs = list of strings that are shorthand names for classes of signal types.  Options are allmag (or all_mag), somemag (or some_mag), asp, aspmag, pci, polarimeter (or polar), cece, and reflectometer (or reflect).
    
    @return nodeList=regular expression that will select for the nodes under \magnetics::top.shoelace.cohere, tfun_allfreq, and trans_fun which correspond to each class of node named in the input list, stringArgs.  Node: the regular expression will only work if the node names are dropped to all lower case.
    
    Ted Golfinopoulos, 8 June 2016
    '''
    nodeRegexList=[]
    if(any([x=="allmag" for x in stringArgs]) or any([x=="all_mag" for x in stringArgs])) :
        nodeRegexList.append('bp.*')

    elif(any([x=="somemag" for x in stringArgs]) or any([x=="some_mag" for x in stringArgs])) :
        nodeList=['bp_ab_top', 'bp_bc_bot', 'bp_bc_top', 'bp_ef_bot', 'bp1t_abk', 'bp1t_ghk', 'bp3t_ghk', 'bp2t_abk', 'bp5t_ghk', 'bp5t_abk', 'bp_ef_top', 'bp_ka_bot', 'bp_ka_top']
        for i in range(0,len(nodeList)):
            nodeRegexList.append(nodeList[i])        

    if(any([x=="asp" for x in stringArgs])) :
        nodeRegexList.append('p\d_.*')

    if(any([x=="aspmag" for x in stringArgs])) :
        nodeRegexList.append('mag.*')

    if(any([x=="pci" for x in stringArgs])) :
        nodeRegexList.append('pci_\d\d')

    if(any([x=="polarimeter" for x in stringArgs]) or any([x=="polar" for x in stringArgs])) :
        nodeRegexList.append('frot_\d\d')

    if(any([x=="cece" for x in stringArgs])) :
        nodeRegexList.append('input_\d\d')

    if(any([re.match(".*reflect.*",x.lower())!=None for x in stringArgs])) :
        nodeRegexList.append('r\d{2,3}ghz.*')
    
    if(any([re.match("gpi",x.lower())!=None for x in stringArgs])) :
        nodeRegexList.append('gpi_\d{1,2}_\d{1,2}')
        
    #Compile regular expressions in list into single regular expression
    nodeRegex='' #Initialize regular expression string
    for i in range(0,len(nodeRegexList)-1) :
        nodeRegex+=nodeRegexList[i]+'|'
    
    #Put in last regular expression without trailing bar, |
    nodeRegex+=nodeRegexList[-1]
