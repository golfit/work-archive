...
def parsePremadeNodeList(stringArgs, s) :
	...
	if(any([re.match(".*reflect.*",x.lower())!=None for x in stringArgs])) :
		...
		nodePath='\\electrons::top.reflect.cpci:dt132_1:' #For example, \ELECTRONS::TOP.REFLECT.CPCI:DT132_1:INPUT_17
		reflectNodes=[nodePath+"input_{0:>02}".format(x) for x in range(17,24+1)]
