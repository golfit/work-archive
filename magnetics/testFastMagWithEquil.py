...
21 February 2016 - changed getstate() to getBoardState() - there have been updates to MDSPlus, and this might be one.
...
# TURN ON EQUILIBRIUM MAGNETICS DEVICES
# See /usr/local/cmod/codes/mhd/magnetics/magshot3.tcl
magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE:DECODER').doMethod('init') #
magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE:J221').doMethod('init') #Need to init J221

#equilDigis=[DT216B(magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE.CPCI_0'+str(ii))) for ii in range(2,6+1)] #Cast digitizer node as acq216 device
equilDigis=[magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE.CPCI_0'+str(ii)) for ii in range(2,6+1)] #Cast digitizer node as acq216 device

print(equilDigis)

#Loop through digitizers and init
for thisDigi in equilDigis :
#for ii in range(2,6+1):
    print(thisDigi)
    thisDigi.getNode('init_action').dispatch() #Initialize device; the initftp method is also listed here: 
    #magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE.CPCI_0'+str(ii)).doMethod('init')

#printAllStates(equilDigis) #Print states of digitizers
...
#Store data for equilibrium magnetics
for thisDigi in equilDigis :
    thisDigi.getNode('store_action').dispatch()
#for ii in range(2,6+1):
    #print(thisDigi)
    #thisDigi.initftp() #Initialize device; the initftp method is also listed here: 
    #magTree.getNode('\MAGNETICS::TOP.CAMAC.HARDWARE.CPCI_0'+str(ii)).doMethod('store')

#printAllStates(equilDigis)
